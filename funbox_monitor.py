...
第一次使用，建議先把 TEST_MODE 設為 True，只跑一次、印出抓到的商品內容，
確認解析結果正確後，再切換成 False 開始真正的長期監控。
"""
...
# True = 只執行一次並印出偵測結果，不會發通知（用來測試解析是否正確）
# False = 進入無限迴圈，每隔 CHECK_INTERVAL_SECONDS 秒檢查一次，並真正發送通知
TEST_MODE = True

CHECK_INTERVAL_SECONDS = 10 * 60  # 建議至少 5~10 分鐘，避免太頻繁造成對方網站負擔
...
def main():
    if TEST_MODE:
        print(f"[{now()}] === 測試模式：只執行一次 ===")
        check_once(send_notify=False)
        print(f"\n[{now()}] 測試完成。請確認上面印出的商品名稱與價格是否正確。")
        print("確認沒問題後，把 TEST_MODE 改成 False，就會開始長期監控並真正發送通知。")
        return

    print(f"[{now()}] === 開始監控 {TARGET_URL} ===")
